...
export class RomReader {
  ...
  async extractChar(cid) {
    ...
    return decodeChar(dat, cid);
  }
}

// Decode raw character DAT bytes → full renderable character (palette + part atlas + assemblies +
// every animation group). This is the ROM-agnostic core: the GDI path (extractChar above) and the Steam
// Collection path (extract_char_dat over Tauri) both feed the same DAT layout in here.
// Beyond the RGBA atlas, we keep the *indexed* atlas (`idx`) and the raw part index buffers (`partIdx`)
// so the editor can re-tint every animation frame live from an edited 16-colour palette.
export async function decodeChar(dat, cid) {
    ...
    // --- GFX1 part pixels: decode → detwiddle → vflip → indexed + RGBA, shelf-pack into atlas ---
    const tiles = [];
    for (let sel = 0; sel < nParts; sel++) {
      ...
      const disp = vflip(detwiddlePal4(raw, w, h), w, h); // right-side-up display order (palette indices)
      const rgba = new Uint8Array(w*h*4);
      for (let i = 0; i < w*h; i++) {
        const c = bank0[disp[i]]; if (!c || disp[i] === 0 || c[3] === 0) continue;
        const p = i<<2; rgba[p]=c[0]; rgba[p+1]=c[1]; rgba[p+2]=c[2]; rgba[p+3]=255;
      }
      tiles.push({ sel, w, h, rgba, idx: disp });
    }

    tiles.sort((a, b) => b.h - a.h);
    const rects = {}; let ax = 0, ay = 0, rowh = 0;
    const placed = [];
    for (const { sel, w, h, rgba, idx } of tiles) {
      if (ax + w > ATLAS_W) { ax = 0; ay += rowh; rowh = 0; }
      rects[sel] = { x: ax, y: ay, w, h };
      placed.push({ ax, ay, w, h, rgba, idx });
      ax += w+1; rowh = Math.max(rowh, h);
    }
    const atlasH = ay + rowh || 1;
    const bundleData = new Uint8ClampedArray(ATLAS_W * atlasH * 4);
    const idxData = new Uint8Array(ATLAS_W * atlasH); // per-pixel palette index (0 = transparent)
    for (const { ax, ay, w, h, rgba, idx } of placed)
      for (let y = 0; y < h; y++) {
        bundleData.set(rgba.subarray(y*w*4, (y+1)*w*4), ((ay+y)*ATLAS_W+ax)*4);
        idxData.set(idx.subarray(y*w, (y+1)*w), (ay+y)*ATLAS_W+ax);
      }

    const bundleImg = (typeof createImageBitmap === 'function')
      ? await createImageBitmap(new ImageData(bundleData, ATLAS_W, atlasH)) : null;

    // --- Animation groups (header slot 5 = ANIMATION_DATA) ---
    const anim = _parseAnimGroups(dat, cid);

    return { lut, asm: { assemblies }, bundle: { w: ATLAS_W, h: atlasH, parts: rects }, bundleImg, bundleData, idxData, anim };
}
