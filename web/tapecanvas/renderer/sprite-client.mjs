...
export class SpriteClient {
  ...
  loadAsmChar(cid) {
    ...
    const p = (async () => {
      try {
        ...
        // COSTUME palette-LUT atlas (optional, out-of-band): PL{hex}_idx.png + _lut.json
        // (tools/extract_gfx1_atlas.py). The idx.png is single-bank (R=palette index 0..15),
        // and _lut.json carries every PALETTE_DATA bank; the renderer selects the costume
        // body bank (bodyBank + costume*8) per draw group. Best-effort — absence keeps the
        // baked-RGB parts.png path. Same asset pair the whole-sprite loadChar() consumes.
        try {
          const [lutR, idxR] = await Promise.all([
            fetch(base + '_lut.json' + bust), fetch(base + '_idx.png' + bust) ]);
          if (lutR.ok && idxR.ok) {
            const lut = await lutR.json();
            const idxImg = await createImageBitmap(await idxR.blob());
            this.asmChars[cid].idxImg = idxImg; this.asmChars[cid].lut = lut;
            console.log('[sprite-client] loaded COSTUME LUT for ASM char', cid,
              (lut.banks ? lut.banks.length : 0), 'banks', idxImg.width + 'x' + idxImg.height);
          }
        } catch (_e) { /* no indexed atlas for this char — baked-RGB path */ }
  ...
  // GARBLE DETECTOR (VENDORED, Steam tape). Returns true if an effect part's baked pixels
  // collapsed to horizontal color BANDS (every non-empty row a single flat color) — the
  // signature of the offline GFX1-LZSS-decode dead-end (dead back-reference into the live
  // 0x0CE60000 scratch -> constant byte runs -> per-4x4-block color collapse). Sampled ONCE
  // per (atlas,partKey) via an offscreen canvas readback, memoized on c._degen. Restricted
  // to SMALL cells (<=48px) so legit large beams/auras are never touched; requires >=3
  // non-empty rows and >=85% of them single-colored (Cable plasma sel14/part31 = 16/16 =
  // 1.0 -> suppressed; muzzle-ball part35 = 25/32 = 0.78 -> kept).
  _isDegeneratePart(c, partKey) {
    if (!c || !c.img || typeof document === 'undefined') return false;
    if (!c._degen) c._degen = new Map();
    if (c._degen.has(partKey)) return c._degen.get(partKey);
    const part = (c.parts && (c.parts[partKey] || c.parts[String(partKey)]));
    // ONLY the row-banded signal is used — it is VALIDATED to separate decode-dead cells from
    // legit art (on Doom sid 149 it flags part 144 and keeps all 33 legit parts; on Cable it
    // flags plasma part 31). A broader white-fraction / extreme-strip guard was TESTED and
    // REJECTED: Storm alone has 865 legit >=50%-pure-white parts (white hair, lightning), so a
    // white guard nukes legit content. Restricted to small cells so legit large/square art is
    // never even sampled. Residual half-white wide strips (Doom 646/647) are left to the re-rip.
    if (!part || part.w < 2 || part.h < 2 || part.w > 48 || part.h > 48) {
      c._degen.set(partKey, false); return false;
    }
    try {
      if (!this._degenCv) {
        this._degenCv = document.createElement('canvas');
        this._degenCtx = this._degenCv.getContext('2d', { willReadFrequently: true });
      }
      const W = part.w, H = part.h;
      this._degenCv.width = W; this._degenCv.height = H;
      this._degenCtx.clearRect(0, 0, W, H);
      this._degenCtx.drawImage(c.img, part.x, part.y, W, H, 0, 0, W, H);
      const data = this._degenCtx.getImageData(0, 0, W, H).data;
      let solidRows = 0, nonEmptyRows = 0;
      for (let y = 0; y < H; y++) {
        let first = -1, uniform = true, anyOpaque = false;
        for (let x = 0; x < W; x++) {
          const i = (y * W + x) * 4;
          if (data[i + 3] < 8) continue;                 // ignore transparent
          anyOpaque = true;
          const rgb = (data[i] << 16) | (data[i + 1] << 8) | data[i + 2];
          if (first < 0) first = rgb; else if (rgb !== first) uniform = false;
        }
        if (anyOpaque) { nonEmptyRows++; if (uniform) solidRows++; }
      }
      const deg = nonEmptyRows >= 3 && (solidRows / nonEmptyRows) >= 0.85;
      c._degen.set(partKey, deg);
      return deg;
    } catch (e) { c._degen.set(partKey, false); return false; }
  }
  ...
  buildEmitterDrawList(canvasW, canvasH) {
    ...
    // DEFECT #2 (2026-08-29): each object's OWN draw layer is threaded into per-item z so a
    // single global z sort reproduces the game's 16-layer bucket walk ACROSS owners (a
    // higher-layer effect/projectile draws in front of a lower-layer body regardless of
    // which fighter owns it). Set true whenever an item's z was derived from a real
    // per-object layer (owner.layer); the sort below uses the global path when it (or engZ)
    ...
    const emitAssembly = (owner, sid) => {
      ...
      if (c.selKeyed) {
        ...
        for (let _ri = 0; _ri < recs.length; _ri++) {
          ...
          // GARBLE GUARD (VENDORED, Steam tape). Effect cells (owner.fxGuard) whose pixels
          // collapsed to horizontal color BANDS are OFFLINE-DECODE garbage baked into the
          // atlas (the _note "no capture": GFX1 LZSS back-references the live 0x0CE60000
          // scratch absent from the static file -> constant runs -> per-4x4-block color
          // collapse -> every row one color = the "smeared arc of streaks" the operator saw
          // on Cable's plasma). Suppress them so the render is CLEAN (correct cells + muzzle
          // flash draw; garbage cells draw nothing). The REAL fix is a re-rip with live
          // PARTDUMP --realparts. Bodies are NEVER guarded (owner.fxGuard only set for fx).
          // window._fxGarbleGuard=false disables (A/B against a re-ripped atlas).
          if (owner.fxGuard && this._isDegeneratePart(c, r.part)) { skipSel++; continue; }
          ...
          const item = { charId: owner.fx ? -1 : owner.cid, slot: owner.slot, z,
            ...
            costume: owner.costume };   // set on bodies only -> routes to the costume-LUT path
          if (owner.blend != null) item.blend = owner.blend;
          // VICTIM HIT-FLASH (render-only, sh4-re 0.3.29). When the owner body is in hitstun
          // (owner.hitFx, driven from the tape — see tape-adapter.applyFrame) tint the body
          // parts near-white additively — the render-only analogue of the engine swapping to
          // hit-flash palette row 25. Bodies only (fx objects never set hitFx).
          if (owner.hitFx && !owner.fx) item.tint = [0.5, 0.45, 0.45];
          out.push(item);
          drawn++;
    ...
    if (!force) for (let s = 0; s < 6; s++) {
      ...
      // BODY GARBLE GUARD: attack poses embed projectile/beam cells (e.g. Doom sid 149's
      // solid-white part 144, the offline-decode dead-end) that render as a white rectangle.
      // The row-banded detector flags ONLY those (verified: it suppresses part 144 and keeps
      ...
      emitAssembly({ cid: sl.char_id, exx, eyy, facing: sl.facing, slot: s, zBase: 0, isBody: true,
                     ...
                     costume: (sl.costume != null ? (sl.costume | 0) : 0), // costume -> body-bank LUT
                     hitFx: (sl.hitFx && this.hitFlashOn !== false) ? 1 : 0,  // victim hit-flash (render-only)
                     fxGuard: bodyGuard },                             // suppress decode-dead cells
                   sl.sprite_id);
    ...
    if (!force && this.objectsOn !== false && satMode !== 'off') for (const o of (this.objects || [])) {
      ...
      // VENDORED-COPY PATCH (Steam tape replay): gated on o.additive (only the tape-adapter sets it),
      // so the live-game path is unchanged; owned here so nothing lands in the maplecast repo.
      if (o.additive) {
        ...
        // fxGuard: suppress offline-decode banded-garbage cells (see _isDegeneratePart).
        const guard = (typeof window === 'undefined') ? true
                    : (window._fxGarbleGuard !== undefined ? !!window._fxGarbleGuard : true);
    ...
    // DEFECT #2: when per-OBJECT ordering exists (engine 1/W in item.z, OR each object's own
    // draw layer folded into item.z via LAYER_BIG), sort the WHOLE list by item.z — the game's
    // global per-object layer walk across ALL owners. Cid-contiguity is NO LONGER needed:
    // sprite-gpu render() batches consecutive same-(route,cid) runs, so cross-owner interleave
    // is drawn correctly. JS sort is stable -> equal-z items keep emission order. Fall back to
    // the legacy per-cid group heuristic (screen-y depth / char_id) ONLY when neither engine z
    // nor a per-object layer was seen (older frames). The knobs still gate each source.
    const perObjZ = (useEngZ && anyEngZ) || (useLayer && sawObjLayer);
    if (perObjZ) {
      out.sort((a, b) => ((a.z || 0) - (b.z || 0)));                 // GLOBAL per-object layer/depth
    } else {
      out.sort((a, b) =>
