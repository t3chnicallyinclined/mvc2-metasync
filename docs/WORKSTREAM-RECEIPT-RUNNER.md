...
| Tape harvest | agent `reader.rs` (rows, nodes, anodes/aobjs, palrows, bg); seam = `read_at` → `mem::Proc::read`; `BLK_SNAP` already serves blk from a byte slice | reusable over runner memory (RENDER §2.1) |
