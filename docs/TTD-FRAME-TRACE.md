...
What it does (`record.ps1`): resolves the pid -> **pre-snapshot** (`dump_live.py`: `pre\meta.json blk.bin blk2.bin
...
3. `calls_f<k>.jsonl` — every call into the tracked function set inside the window (`TTD.Calls(addr,...)` in
   chunks of `--calls-chunk 200`): `{t, te, fn, rva, ret, ret_rva, tid}`. Default set = the 10,794 game-range
   functions from `d3dcap/replay/re_map/steam_funcs.jsonl` (`--all-funcs` for all 29,678).
...
6. `dump_f<k>_{blk,ctx,dcram,game_state}.bin` — memory **at the first window boundary** (`SeekTo` + page-wise
   ...
   = trace pages with holes filled from `pre\` (`*.merge.json` says which). These are the emu-harness inputs.
...
Status 2026-09-02: PROVEN without elevation — winget installs, `ttd.exe` runs, `dump_live.py` snapshots the live game
(all images fully readable), `cdbX64 .scriptload extract.js` loads and the FileSystem/JSON/readMemory selftest
passes on a live target; the static part of the seed (GGPO callback pairs, section 0) was applied to the live graph 2026-09-02 after backup `re_kb_data/_exports/re_kb_20260902-235523.surql` (16 statements, 0 failed). **NOT yet proven** (needs the one UAC click this session could not give): the attach/stop
cycle and the TTD `Memory`/`Calls` queries on a real `.run`. `probe.json` is written first precisely so a property-name
mismatch on the first real replay is a 1-line fix, not a mystery.
