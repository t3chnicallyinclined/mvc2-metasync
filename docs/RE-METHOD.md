...
| HUD portrait / name-plate pages: TCW 0xC9A..0xCA5 are twelve PER-CHARACTER slots rewritten at every match load by `FUN_14060d560` from AFS `3+cid` (16-bit LZSS `FUN_140611e90`); portrait = 0xC9A + `DAT_140a6aac8[slot]` from page `DAT_140a6aac4[*(fighter+0x655)]` (= the tape's `assist[slot]`), name = 0xCA0 + same k from page 2; slot EVEN = P1 / ODD = P2. Resolving them from the capture TCW library drew a foreign roster on every tape. Gate 29/29 captured pages byte-exact from the DATs; emitter L1 5/5, L3 30/30 | `docs/PORTRAIT-PAGES-GHIDRA.md`, seed `116_portrait_pages.surql`, `d3dcap/replay/rip_portraits.py --gate` |
