...
const RUNTIME_DIR: &str = "C:\\g";
const SKINS_DAT: &str = "C:\\g\\skins.dat";

const CHARS_RUNTIME: &str = "C:\\g\\chars.json";
...
fn capture_live_legacy() -> Result<Vec<String>, String> {
    fs::create_dir_all(RUNTIME_DIR).ok();
    fs::write("C:\\g\\dump.txt", "1").ok();
    let dump = fs::read_to_string("C:\\g\\pal_dump.txt").unwrap_or_default();
    let mut out = Vec::new();
    let mut seen = std::collections::HashSet::new();
...
#[tauri::command]
fn apply_multi(entries: Vec<SkinEntry>) -> Result<String, String> {
    fs::create_dir_all(RUNTIME_DIR).ok();
    let mut out = String::from("# live skins\n");
    for e in &entries { if e.palette.len() >= 16 { out.push_str(&sigs_lines(&e.sigs, &e.palette)); } }
    fs::write(SKINS_DAT, out).map_err(|e| e.to_string())?;
    Ok(format!("{} skin(s) active", entries.len()))
}
...
fn reset_all() -> Result<String, String> {
    ...
    let _ = fs::write("C:\\g\\effect.txt", "0 80");
    let _ = fs::write("C:\\g\\effect_targets.txt", "");
...
fn set_effect(mode: u8, speed: i32) -> Result<String, String> {
    fs::create_dir_all(RUNTIME_DIR).ok();
    fs::write("C:\\g\\effect.txt", format!("{} {}", mode, speed)).map_err(|e| e.to_string())?;
...
fn set_effect_target(character: String) -> Result<String, String> {
    ...
    fs::write("C:\\g\\effect_targets.txt", lines.join("\n")).map_err(|e| e.to_string())?;
...
"#;
    ...
    match s.as_str() {
        ...
        p => { fs::create_dir_all(RUNTIME_DIR).ok(); let _ = fs::write("C:\\g\\rom_path.txt", p); Ok(p.to_string()) }
    }
}
...
fn set_rom_path(path: String) -> Result<String, String> {
    ...
    let _ = fs::write("C:\\g\\rom_path.txt", &s);
    Ok(s)
}
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![apply_skin, clear_skin, learn_character, capture_live, apply_sigs, apply_multi, reset_all, reset_hook_regions, set_effect, set_effect_target, detect_rom, set_rom_path, get_rom_path,
            ...
            sync::contest_match, sync::result_notifications,
            rom::rom_size, rom::rom_read, rom::rom_write, rom::rom_backup, rom::rom_prepare, rom::bake_palette, rom::read_char_palette, rom::extract_char_dat])
